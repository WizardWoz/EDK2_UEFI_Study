...
#在.dsc文件中必须被第一个定义，
[Defines]
  PLATFORM_NAME                  = AppPkg									#必须    
  PLATFORM_GUID                  = 0458dade-8b6e-4e45-b773-1b27cbda3e06		#必须
  PLATFORM_VERSION               = 0.01										#必须
  DSC_SPECIFICATION              = 0x00010006								#必须
  OUTPUT_DIRECTORY               = Build/AppPkg								#可选
  SUPPORTED_ARCHITECTURES        = IA32|X64|ARM|AARCH64						#必须
  BUILD_TARGETS                  = DEBUG|RELEASE|NOOPT						#必须
  SKUID_IDENTIFIER               = DEFAULT									#必须

#
#  Debug output control
#  通过DEFINE定义的宏是全局的，fdf文件和!include包含的文件都可通过$(MACRO)来访问
#
  DEFINE DEBUG_ENABLE_OUTPUT      = FALSE       # Set to TRUE to enable debug output  #DEFINE，可选
  DEFINE DEBUG_PRINT_ERROR_LEVEL  = 0x80000040  # Flags to control amount of debug output #DEFINE，可选
  DEFINE DEBUG_PROPERTY_MASK      = 0           #DEFINE，可选
